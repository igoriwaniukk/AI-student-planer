...
app.post('/api/plan/rescue', async (req, res) => {
  ...
  const { tasks, energy, availableMinutes, reasons, lang } = req.body || {};
  if (!Array.isArray(tasks) || tasks.length === 0) {
    res.status(400).json({ error: 'Brak zadań do rozdysponowania.' });
    return;
  }
  if (typeof availableMinutes !== 'number' || availableMinutes < 0) {
    res.status(400).json({ error: 'Brak dostępnego czasu (availableMinutes).' });
    return;
  }
  ...
  try {
    ...
    const toolUse = response.content.find((b) => b.type === 'tool_use');
    if (!toolUse) {
      res.status(502).json({ error: 'Claude nie zwrócił propozycji ratunku.' });
      return;
    }
    res.json({ blocks: toolUse.input.blocks || [], moved: toolUse.input.moved || [], rationale: toolUse.input.rationale || '' });
  } catch (err) {
    if (err instanceof Anthropic.APIError) {
      res.status(err.status || 502).json({ error: err.message || 'Błąd po stronie Claude.' });
      return;
