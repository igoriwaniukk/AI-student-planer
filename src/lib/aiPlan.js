...
export function toValidatedSchedule(blocks, ids, taskDefs, durOverride) {
  ...
  for (const block of blocks) {
    ...
    const conflict = checkBlockConflict(block.taskId, block.start, dur, schedule, (id) => taskDefs.find((t) => t.id === id));
...
export async function requestAIPlan({ taskDefs, tasks, taskState, energy, pref, durOverride }) {
  ...
  try {
    ...
    const schedule = toValidatedSchedule(data.blocks, ids, taskDefs, durOverride);
