...
export default function TaskEditSheet({ planner }) {
  ...
  return (
    <BottomSheet maxHeight="92%">
      <div style={{ fontSize: 17, fontWeight: 750, letterSpacing: '-.01em' }}>Edytuj zadanie</div>
      <div style={{ fontSize: 12, color: '#7a7a8a', marginTop: 6 }}>{fm.subject} — {fm.name}</div>

      <div style={{ fontSize: 11, fontWeight: 750, letterSpacing: '.08em', color: '#7a7a8a', margin: '18px 0 9px' }}>NAZWA ZADANIA</div>
      <input
        value={fm.name}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {SUBJECTS.map((v) => (
          <Chip key={v} label={v} active={fm.subject === v} onClick={() => patchTaskEdit({ subject: v })} style={{ padding: '9px 13px' }} />
        ))}
      </div>
      ...
      <div style={{ fontSize: 11, fontWeight: 750, letterSpacing: '.08em', color: '#7a7a8a', margin: '18px 0 9px' }}>PRIORYTET</div>
      ...
      <div style={{ fontSize: 11, fontWeight: 750, letterSpacing: '.08em', color: '#7a7a8a', margin: '18px 0 9px' }}>NOTATKA (OPCJONALNIE)</div>
      <textarea
        value={fm.note}
        onChange={(e) => patchTaskEdit({ note: e.target.value })}
        placeholder="Np. zrobić zadania 4–7"
      ...
      <div style={{ display: 'flex', gap: 11, marginTop: 18, paddingBottom: 8 }}>
        <div onClick={cancelTaskEdit} style={{ flex: 1, height: 50, borderRadius: 15, background: 'rgba(255,255,255,.055)', border: '1px solid rgba(255,255,255,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 650, cursor: 'pointer' }}>Anuluj</div>
        <div onClick={saveTaskEdit} style={{ flex: 1.4, height: 50, borderRadius: 15, background: 'linear-gradient(160deg,#8b6dff,#6d4dff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Zapisz zmiany</div>
      </div>
    </BottomSheet>
  );
}
