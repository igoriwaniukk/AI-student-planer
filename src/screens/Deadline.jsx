...
export default function Deadline({ planner }) {
  ...
  return (
    <div className="sc" style={{ height: '100%', overflowY: 'auto', padding: '56px 20px 116px' }}>
      ...
      <div style={{ fontSize: 29, fontWeight: 750, letterSpacing: '-.025em', marginTop: 20 }}>Dodaj termin</div>
      <div style={{ fontSize: 13, lineHeight: 1.5, color: '#8a8a99', marginTop: 8 }}>Dodaj sprawdzian lub inne zobowiązanie, a AI pomoże rozłożyć przygotowanie.</div>

      <LabelRequired label="Rodzaj terminu" />
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {KINDS.map((k) => <Chip key={k} label={k} active={state.kind === k} onClick={() => setField('kind', k)} />)}
      </div>

      <LabelRequired label="Przedmiot" />
      ...
      />
      {nameEmpty && <div style={{ fontSize: 12, color: '#f5a524', marginTop: 8 }}>Wpisz nazwę terminu.</div>}

      <LabelRequired label="Data terminu" />
      <div style={{ display: 'flex', gap: 10 }}>
        <div onClick={() => setField('dateValid', !state.dateValid)} style={{ flex: 2, height: 66, borderRadius: 15, background: 'rgba(255,255,255,.045)', border: '1px solid ' + (state.dateValid ? 'rgba(255,255,255,.09)' : 'rgba(245,165,36,.5)'), padding: '0 14px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4, cursor: 'pointer' }}>
          <span style={{ fontSize: 9.5, fontWeight: 750, letterSpacing: '.1em', color: '#7a7a8a' }}>DATA</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: state.dateValid ? '#f4f4f7' : '#f5a524' }}>{state.dateValid ? 'Piątek, 31 lipca 2026' : 'Piątek, 17 lipca 2026'}</span>
        </div>
        <div style={{ flex: 1, height: 66, borderRadius: 15, background: 'rgba(255,255,255,.045)', border: '1px solid rgba(255,255,255,.09)', padding: '0 14px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
          <span style={{ fontSize: 9.5, fontWeight: 750, letterSpacing: '.1em', color: '#7a7a8a' }}>GODZINA</span>
          <span style={{ fontSize: 17, fontWeight: 750 }}>09:00</span>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 10 }}>
        <span style={{ fontSize: 11.5, fontWeight: 650, color: state.dateValid ? '#8ff0de' : '#f5a524', padding: '5px 10px', borderRadius: 8, background: state.dateValid ? 'rgba(46,230,197,.13)' : 'rgba(245,165,36,.13)' }}>{state.dateValid ? 'Za 11 dni' : 'Data minęła'}</span>
        {!state.dateValid && <span style={{ fontSize: 12, color: '#f5a524' }}>Wybierz przyszłą datę.</span>}
      </div>

      <LabelRequired label="Zakres materiału" tag="zalecane" tagColor="#8ff0de" />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {state.topics.map((t, i) => (
          <div key={i} style={{ minHeight: 52, borderRadius: 15, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', padding: '13px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ flex: 1, fontSize: 14, fontWeight: 650, lineHeight: 1.35 }}>{t}</span>
            <span onClick={() => removeTopic(i)} style={{ fontSize: 12.5, fontWeight: 650, color: '#8a8a99', cursor: 'pointer', padding: '4px 6px' }}>Usuń</span>
          </div>
        ))}
      </div>
      <div onClick={addTopic} style={{ marginTop: 10, height: 48, borderRadius: 15, border: '1.5px dashed rgba(255,255,255,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13.5, fontWeight: 650, color: '#9a9aab', cursor: 'pointer' }}>+ Dodaj temat</div>
      {state.topicErr && topicsMissing && <div style={{ fontSize: 12, color: '#f5a524', marginTop: 8 }}>Dodaj przynajmniej jeden temat do planu przygotowania.</div>}

      <div style={{ fontSize: 15, fontWeight: 700, margin: '22px 0 11px' }}>Jak trudny jest ten materiał?</div>
      ...
      <div style={{ fontSize: 15, fontWeight: 700, margin: '22px 0 11px' }}>Jak dobrze znasz już ten materiał?</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {LEVELS.map((l, i) => <ListRow key={l} label={(i + 1) + '. ' + l} active={state.level === i + 1} onClick={() => setField('level', i + 1)} last={i === LEVELS.length - 1} />)}
      </div>
      <div style={{ fontSize: 11.5, lineHeight: 1.45, color: '#7a7a8a', marginTop: 10 }}>Ta ocena pomaga ustalić liczbę i długość sesji. Możesz ją później zmienić.</div>

      <div style={{ fontSize: 15, fontWeight: 700, margin: '22px 0 11px' }}>Jaki wynik chcesz osiągnąć?</div>
      ...
      <div style={{ fontSize: 11.5, color: '#7a7a8a', marginTop: 10 }}>To Twój cel, a nie gwarancja wyniku.</div>
      ...
      <div onClick={() => setField('autoPlan', !state.autoPlan)} style={{ marginTop: 20, padding: 15, borderRadius: 18, background: 'rgba(255,255,255,.035)', border: '1px solid rgba(255,255,255,.07)', display: 'flex', gap: 13, cursor: 'pointer' }}>
        ...
        <div>
          <div style={{ fontSize: 13.5, fontWeight: 700, lineHeight: 1.35 }}>Utwórz automatyczny plan przygotowania</div>
          <div style={{ fontSize: 11.5, lineHeight: 1.45, color: '#7a7a8a', marginTop: 5 }}>AI podzieli materiał na krótkie sesje, powtórki i samosprawdzenie.</div>
        </div>
      </div>

      <div style={{ marginTop: 16, padding: 16, borderRadius: 20, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)' }}>
        <div style={{ fontSize: 9.5, fontWeight: 750, letterSpacing: '.1em', color: '#7a7a8a' }}>PODSUMOWANIE TERMINU</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginTop: 14 }}>
          <Row label="Rodzaj" value={state.kind} />
          <Row label="Przedmiot" value={state.subject} />
          <Row label="Termin" value="31 lipca, 09:00" />
          <Row label="Pozostało" value={state.dateValid ? 'Za 11 dni' : 'Data minęła'} />
          <Row label="Zakres" value={state.topics.length === 1 ? '1 temat' : state.topics.length + ' tematy'} />
          <Row label="Trudność" value={'Trudność: ' + (state.difficulty === 'Łatwy' ? 'Łatwa' : state.difficulty === 'Trudny' ? 'Trudna' : 'Średnia')} />
          <Row label="Znajomość" value={'Przygotowanie: ' + state.level + ' z 5'} />
          <Row label="Cel" value={'Cel: ' + state.goal} />
        </div>
        <div style={{ height: 1, background: 'rgba(255,255,255,.07)', margin: '15px -16px' }} />
        <div style={{ display: 'flex', gap: 9 }}>
          <span style={{ color: '#35d07f', fontSize: 12 }}>✓</span>
          <span style={{ fontSize: 12.5, lineHeight: 1.45, color: '#5fdd9b' }}>{state.autoPlan ? 'Jest wystarczająco dużo czasu, aby rozłożyć przygotowanie.' : 'Zapiszemy sam termin bez sesji przygotowania.'}</span>
        </div>
      </div>
      ...
      <StickyFooter>
        ...
        >
          {state.autoPlan ? 'Zapisz i utwórz plan' : 'Zapisz termin'}
        </div>
      </StickyFooter>

      {state.deadlineOnlySaved && (
        <ConfirmCard title="Termin został zapisany." sub="Nie dodaliśmy żadnych sesji przygotowania." onDone={goHomeDeadline} />
      )}
    </div>
  );
}
