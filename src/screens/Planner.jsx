  ...
  return (
    <div className="sc" style={{ height: '100%', overflowY: 'auto', padding: '56px 20px 176px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 750, letterSpacing: '.12em', color: '#c9baff', padding: '8px 14px', borderRadius: 999, background: 'rgba(124,92,255,.16)', border: '1px solid rgba(124,92,255,.45)' }}>AI PLANNER</span>
        <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(150deg,#8b6dff,#6d4dff)' }} />
      </div>

      <div style={{ fontSize: 29, fontWeight: 750, letterSpacing: '-.025em', marginTop: 22 }}>Zaplanuj jutro</div>
      <div style={{ fontSize: 13.5, fontWeight: 650, color: '#c9c9d6', marginTop: 8 }}>Poniedziałek, 20 lipca</div>
      <div style={{ fontSize: 13, color: '#8a8a99', lineHeight: 1.45, marginTop: 6 }}>Sprawdź obowiązki i powiedz, jak wygląda Twój dzień.</div>
      ...
      <div style={{ marginTop: 18, padding: 14, borderRadius: 18, background: 'rgba(255,255,255,.035)', border: '1px solid rgba(255,255,255,.07)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 13.5, fontWeight: 700 }}>Jutro masz już zaplanowane</span>
          </div>
          <span style={{ fontSize: 12.5, fontWeight: 650, color: '#a58cff' }}>Edytuj</span>
        </div>
        <div style={{ fontSize: 12, color: '#7a7a8a', marginTop: 7 }}>AI nie zmieni ich godzin.</div>
        <div style={{ height: 1, background: 'rgba(255,255,255,.07)', margin: '13px -14px 0' }} />
        {[['🏫', 'Szkoła', 'Plan lekcji', '8:00–14:40'], ['🎾', 'Tenis', 'Stałe zajęcie', '18:00–19:00']].map(([icon, title, sub, time]) => (
      ...
      <div style={{ fontSize: 17, fontWeight: 750, letterSpacing: '-.01em', margin: '22px 0 12px' }}>Co chcesz zrobić?</div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {d.id === 'math' && <div style={{ fontSize: 11.5, color: '#7a7a8a', marginTop: 8 }}>Gotowość: 45%</div>}
              </div>
              <span onClick={(e) => { e.stopPropagation(); openTaskEdit(d.id); }} style={{ fontSize: 12, fontWeight: 650, color: '#a58cff', cursor: 'pointer' }}>Edytuj</span>
            </div>
          </div>
        );
      })}

      <div style={{ marginTop: 12, height: 50, borderRadius: 16, border: '1.5px dashed rgba(255,255,255,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13.5, fontWeight: 650, color: '#9a9aab', cursor: 'pointer' }}>+ Dodaj zadanie</div>

      <div style={{ marginTop: 16, padding: 16, borderRadius: 20, background: 'rgba(255,255,255,.035)', border: '1px solid rgba(255,255,255,.07)' }}>
        <div style={{ fontSize: 16.5, fontWeight: 750, letterSpacing: '-.01em' }}>Kiedy masz czas?</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 14 }}>
          {['15:30', '21:30'].map((t, i) => (
            <div key={t} style={{ flex: 1 }}>
              <div style={{ fontSize: 9.5, fontWeight: 750, letterSpacing: '.1em', color: '#7a7a8a', marginBottom: 7 }}>{i === 0 ? 'OD' : 'DO'}</div>
              <div style={{ height: 56, borderRadius: 15, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.09)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 14px' }}>
                <span style={{ fontSize: 20, fontWeight: 750, letterSpacing: '-.01em' }}>{t}</span>
        ...
        <div style={{ marginTop: 14, padding: 13, borderRadius: 15, background: 'rgba(53,208,127,.07)', border: '1px solid rgba(53,208,127,.22)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#35d07f' }} /><span style={{ fontSize: 13, fontWeight: 700, color: '#5fdd9b' }}>5 godz. wolnego</span></div>
          <div style={{ fontSize: 12, color: '#a3a3b3', marginTop: 6 }}>15:30–18:00 oraz 19:00–21:30</div>
          <div style={{ fontSize: 11.5, color: '#7a7a8a', marginTop: 4 }}>AI uwzględni przerwy i pozostawi bufor.</div>
        </div>
      </div>

      <div style={{ fontSize: 16.5, fontWeight: 750, letterSpacing: '-.01em', margin: '22px 0 12px' }}>Ile energii przewidujesz?</div>
      <EnergyPicker value={state.energy} onChange={(v) => update({ energy: v })} />

      <div style={{ fontSize: 14.5, fontWeight: 700, margin: '20px 0 11px' }}>Na czym najbardziej Ci zależy?</div>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, margin: '20px 0 10px' }}>
        <span style={{ fontSize: 14.5, fontWeight: 700 }}>Dodatkowa uwaga dla planera</span>
        <span style={{ fontSize: 12, color: '#7a7a8a' }}>(opcjonalnie)</span>
      </div>
      <textarea
        placeholder="Na przykład: po tenisie mogę mieć mniej energii."
      ...
      <div style={{ marginTop: 16, padding: 16, borderRadius: 20, background: 'rgba(255,255,255,.035)', border: '1px solid rgba(255,255,255,.07)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 14 }}>
          <Row label="Zadania" value={nTasks === 1 ? '1 zadanie' : nTasks + ' zadania'} />
          <Row label="Czas nauki" value={sumTime} />
          <Row label="Dostępny czas" value="5 godz." />
          <Row label="Energia" value={state.energy} />
          <Row label="Preferencja" value={state.pref} />
        </div>
        <div style={{ height: 1, background: 'rgba(255,255,255,.07)', margin: '15px -16px' }} />
        <div style={{ display: 'flex', gap: 9 }}>
          <span style={{ color: '#35d07f', fontSize: 12 }}>✓</span>
          <span style={{ fontSize: 12.5, lineHeight: 1.45, color: '#a3a3b3' }}>Wszystkie wybrane zadania powinny zmieścić się w planie.</span>
        </div>
      </div>

      <StickyFooter>
        <PrimaryButton onClick={generatePlan}>Ułóż mój plan</PrimaryButton>
        <div style={{ textAlign: 'center', fontSize: 11.5, color: '#7a7a8a', marginTop: 11 }}>Przed zatwierdzeniem możesz zmienić każdą część planu.</div>
      </StickyFooter>
